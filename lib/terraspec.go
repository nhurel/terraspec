...
func ExecTerraspec(specDir string, displayPlan bool, tfVersion string) int {
	...
	tsCtx := &Context{TerraformVersion: tfversion.SemVer, UserVersion: newSemVer}
...
func runTestCase(tc *testCase, tsCtx *Context, displayPlan bool, results chan<- *testReport) {
...
func PrepareTestSuite(dir string, tc *testCase, tsCtx *Context) (*terraform.Context, *Spec, tfdiags.Diagnostics) {
	...
	providerResolver, err := BuildProviderResolver(absDir)
	if err != nil {
		ctxDiags = ctxDiags.Append(err)
		return nil, nil, ctxDiags
	}
